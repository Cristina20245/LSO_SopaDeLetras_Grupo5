...
public class SopaDeLetras {
    ...
    public static boolean buscarPalabra(String palabra) {
        ...
        for (int i = 0; i < sopa.length; i++) {
            for (int j = 0; j < sopa[i].length; j++) {
                if (Character.toLowerCase(sopa[i][j]) == palabra.charAt(0)) {
                    // Comprobar horizontal
                    int[][] coords = comprobarPalabra(palabra, i, j, false);
                    if (coords != null) {
                        marcarLetrasDescubiertas(coords);
                        palabrasEncontradas++;
                        if (palabrasEncontradas == 5) // Salir después de encontrar 5 palabras
                            return true;
                    }
                    // Comprobar vertical
                    coords = comprobarPalabra(palabra, i, j, true);
                    if (coords != null) {
                        marcarLetrasDescubiertas(coords);
                        palabrasEncontradas++;
                        if (palabrasEncontradas == 5) // Salir después de encontrar 5 palabras
    ...
    public static int[][] comprobarPalabra(String palabra, int i, int j, boolean vertical) {
        ...
        while (k < palabra.length()) {
            if (i < sopa.length && j < sopa[i].length && Character.toLowerCase(sopa[i][j]) == palabra.charAt(k)) {
                coords[k][0] = i;
                coords[k][1] = j;
                if (vertical)
                    i++;
...
//A partir de aquí, este código es el original de la práctica
